      ...
      try {
        await fetch("/api/notificar", {
          ...
          body: JSON.stringify({
            ...
            mensagem: `${nome.split(" ")[0]} se cadastrou e aguarda aprovação.`,
            url: `${window.location.origin}/admin/pedidos`,
          }),
        });
      } catch {
        // Notificação falhou mas não bloqueia o cadastro
      }
    }

    setSucesso("Cadastro realizado! Aguarde a aprovação do administrador.");
    setLoading(false);
  }
...
function Spinner({ dark = false }: { dark?: boolean }) {
  return (
    <span className="flex items-center justify-center gap-2">
      <span
        className={`w-4 h-4 border-2 border-t-transparent rounded-full animate-spin ${dark ? "border-white" : "border-black"}`}
      />
      Aguarde...
    </span>
  );
